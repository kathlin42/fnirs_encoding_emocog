...
for (contrast in contrasts){
  ...
  write.csv(table, file.path(path, "derivatives", "fnirs_preproc", setting, 'LMM_coefficients', paste("coefficients_", contrast, "_hbo.csv", sep="")))
}
...
for (contrast in contrasts){
  ...
  upper_ci = data.table("0.975]" = confint[[2]][3:nrow(confint)])
  table <- cbind(table, lower_ci, upper_ci)
  write.csv(table, file.path(path, "derivatives", "fnirs_preproc", setting, 'LMM_coefficients', paste("coefficients_", contrast, "_hbr.csv", sep="")))
}
